...
export function RecentBidFeed({ bids, meId }: { bids: BidEvent[]; meId: string }) {
  return (
      ...
      {bids.length ? (
        bids.slice(0, 8).map((bid, index) => (
          <div key={bid.id || `${bid.userId}-${index}`} className={bid.accepted === false ? 'rejected' : 'accepted'}>
            ...
            <small>
              {bid.accepted === false ? businessErrorMessage(bid.rejectReason) || '出价失败' : '有效出价'} · {formatEventTime(bid.createdAtUnixMs)}
            </small>
          </div>
        ))
      ) : (
