...
export function noticeForAuctionEvent(event: AuctionSocketEvent, currentUserId: string, previousLeaderId?: string): string {
  ...
  if (event.type === AUCTION_EVENT_TYPE.BID_REJECTED) {
    return businessErrorMessage(event.rejectReason || event.reason, { lot: event.lot }) || '出价失败，请调整金额后重试';
  }
  ...
  if (event.type === AUCTION_EVENT_TYPE.LOT_CANCELLED) return businessErrorMessage(event.reason) || event.reason || '本场竞拍已取消';
