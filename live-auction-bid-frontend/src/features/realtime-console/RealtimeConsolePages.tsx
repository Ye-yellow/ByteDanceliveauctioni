...
export function BidAuditPage({ roomId }: { roomId: string }) {
  ...
  return <section className="postLivePage bidAuditPage">
    <StudioCard padding="lg" className="postLiveHeader bidAuditHero"><StudioPageHeader eyebrow="Bid audit" title="出价明细" description="最近出价来自 room snapshot；后端未返回的延迟和幂等 Key 不在页面伪造。" actions={<StudioButton type="button" variant="secondary" icon={<RefreshCw size={15} />} loading={loading} onClick={() => void sync()}>重新同步</StudioButton>} /></StudioCard>
    {error ? <div className="auctionMgmtNotice danger"><AlertTriangle size={16} />{error}</div> : null}
